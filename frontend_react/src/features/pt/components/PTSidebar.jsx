...
export default function PTSidebar() {
  ...
  return (
    <aside className={styles.ptSidebar}>
      <button className={styles.mobileClose} onClick={() => window.dispatchEvent(new CustomEvent('closePTMenu'))}>✕</button>

      <div className={styles.ptBrand}>
        <NavLink to="/pt" style={{textDecoration:'none', color: 'inherit'}}>
          PT Portal
        </NavLink>
      </div>

      <nav className={styles.ptNav}>
        <NavLink to="/pt" end className={({isActive})=> isActive? styles.active:''}>
          <Icon name="dashboard"/> Dashboard
        </NavLink>
        <NavLink to="/pt/profile" className={({isActive})=> isActive? styles.active:''}>
          <Icon name="profile"/> Thông tin của tôi
        </NavLink>
        <NavLink to="/pt/packages" className={({isActive})=> isActive? styles.active:''}>
          <Icon name="packages"/> Gói tập của tôi
        </NavLink>
        <NavLink to="/pt/clients" className={({isActive})=> isActive? styles.active:''}>
          <Icon name="clients"/> Học viên của tôi
        </NavLink>
        <NavLink to="/pt/schedule" className={({isActive})=> isActive? styles.active:''}>
          <Icon name="schedule"/> Lịch làm việc
        </NavLink>
        <NavLink to="/pt/settings" className={({isActive})=> isActive? styles.active:''}>
          <Icon name="settings"/> Cài đặt
        </NavLink>
      </nav>

      <div className={styles.ptProfile}>
        <div className={styles.avatar}>{displayName[0]?.toUpperCase()}</div>
        <div className={styles.profileMeta}>
          <div className={styles.name}>{displayName}</div>
          <div className={styles.role}>Personal Trainer</div>
        </div>
      </div>
    </aside>
  );
