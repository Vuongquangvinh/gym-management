...
export class CheckinModel {
  ...
  static async getAll(filters = {}, limit = 10, startAfterDoc = null) {
    try {
      ...
      if (searchTerm) {
        ...
        try {
          queryConstraints.push(where("searchTokens", "array-contains", searchValue));
        } catch (error) {
      ...
      if (searchTerm && checkins.length === 0) {
        ...
        this.updateMissingSearchTokens().catch(err => {
  ...
  static async update(id, updateData) {
    ...
    const allowedFields = ['checkedAt', 'locationId', 'memberId', 'memberName', 'memberPhone', 'packageId', 'source'];
    const filteredData = {};
  ...
  static normalizeCheckin(data) {
    ...
    let checkedAt = data.checkedAt;
    if (checkedAt && checkedAt.toDate) {
      // Nếu là Firestore Timestamp
      checkedAt = checkedAt.toDate().toISOString();
  ...
  static async updateAllSearchTokensWithPrefix() {
    try {
      ...
      console.log(`Found ${snapshot.docs.length} documents to update with prefix support`);
      ...
      console.log(`Preparing to update ${batch.length} documents with prefix support`);
      
      // Process in smaller batches to avoid hitting limits
      const BATCH_SIZE = 500;
      for (let i = 0; i < batch.length; i += BATCH_SIZE) {
        const currentBatch = batch.slice(i, i + BATCH_SIZE);
        await Promise.all(currentBatch.map(item => 
          updateDoc(item.ref, { 
  ...
  static async updateMissingSearchTokens() {
    try {
      ...
      if (toUpdate.length > 0) {
        const promises = toUpdate.slice(0, 5).map(async (docId) => { // Chỉ update 5 docs mỗi lần
          try {
  ...
  static async updateSearchTokensWithPhone() {
    try {
      ...
      console.log(`Found ${snapshot.docs.length} checkin documents to update with phone support`);
      ...
      for (const docSnap of snapshot.docs) {
        ...
        if (data.memberName) {
          ...
          updated++;
          console.log(`Updated document ${docSnap.id} with ${uniqueTokens.length} search tokens`);
        }
      }
